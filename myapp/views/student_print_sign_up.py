...
def student_print_sign_up(request, activity_id):
    ...
    identity_front = (
        ...
        else InlineImage(doc, user.get_identity_front, width=Mm(80))
    )
    identity_back = (
        identity_back_text
        if user.identity_back == ""
        else InlineImage(doc, user.get_identity_back, width=Mm(80))
    )
    ...
    generate_pdf(
        os.path.join(os.getcwd(), "static", f"{file_name}.docx"),
